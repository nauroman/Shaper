...
namespace Flashunity.Shaper
{
    ...
    {
        public int[] Extrude(Mesh mesh, int[] selectedMeshTriangles, int[] selectedMeshTrianglesFirstIndices, Vector3[] edgesVerticesPairs, Vector2[] edgesUVPairs, Vector3 direction)
        {
            ...
            for (int i = 0; i < edgesVerticesPairs.Length; i += 2)
            {
                ...
                var uv0 = edgesUVPairs [i];
                var uv1 = edgesUVPairs [i + 1];
                var uv2 = new Vector2(uv1.x, uv1.y);
                var uv3 = new Vector2(uv0.x, uv0.y);
...
//                var n1 = GetNormal(v0, v3, v2);
            ...
            UpdateMesh(mesh, meshVertices.ToArray(), meshTriangles.ToArray(), meshNormals.ToArray(), meshUV.ToArray());
...
//            var perpLength = v.magnitude;
        ...
        */
        ...
        void UpdateMesh(Mesh mesh, Vector3[] vertices, int[] triangles, Vector3[] normals, Vector2[] uv)
        {
            mesh.Clear();
